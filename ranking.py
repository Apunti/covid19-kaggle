...
class Ranking:
    ...
    def _get_dictionaries(self, path):
        ...
        if self.data == 'abstracts':
            with open(path + 'document_frequencies_abstract_v8.p', 'rb') as fp:
                self.document_frequencies = pickle.load(fp)

            with open(path + '/term_frequencies_abstract_v8.p', 'rb') as fp:
                self.term_frequencies = pickle.load(fp)

            with open(path + '/document_length_abstract_v8.p', 'rb') as fp:
                self.document_length = pickle.load(fp)
        else:
            with open(path + '/document_frequencies_text_v8.p', 'rb') as fp:
                self.document_frequencies = pickle.load(fp)

            with open(path + '/term_frequencies_text_v8.p', 'rb') as fp:
                self.term_frequencies = pickle.load(fp)

            with open(path + '/document_length_text_v8.p', 'rb') as fp:
                self.document_length = pickle.load(fp)
